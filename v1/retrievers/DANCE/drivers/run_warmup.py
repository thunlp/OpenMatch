...
from model.models import MSMarcoConfigDict,MSMarcoConfig,RobertaDot_NLL_LN
...
def train(args, model, tokenizer, f, train_fn):
    ...
    #     tr_loss, logging_loss = 0.0, 0.0
    # else:
        # tr_loss, logging_loss = 0.0, 0.0 # primary task loss
        # tr_loss_dual, logging_loss_dual = 0.0, 0.0 # dual task loss
        # tr_loss_total, logging_loss_total = 0.0, 0.0 # sum of two task losses
    ...
    for m_epoch in train_iterator:
        ...
        if args.data_loader_num_workers > 0:
            if args.dual_training:
                training_data_path= args.data_dir+"/quadruplets.train.small.tsv"
            else:
                training_data_path= args.data_dir+"/triples.train.small.tsv"
                

            sds = StreamingDataset(f,train_fn,distributed=False,
                num_workers=args.data_loader_num_workers,file_name=training_data_path)
        else:
            sds = StreamingDataset(f,train_fn,distributed=False,num_workers=0)
        epoch_iterator = DataLoader(sds, batch_size=args.per_gpu_train_batch_size, num_workers=args.data_loader_num_workers)
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # for triple_process_fn 
            inputs = {
                    "query_ids": batch[0],
                    "attention_mask_q": batch[1],
                    "input_ids_a": batch[2],
            ...
            if args.dual_training:
                # for  quadruplet_process_fn(line, i, tokenizer, args)
                inputs["neg_query_ids"] = batch[6]
                inputs["attention_mask_neg_query"] = batch[7]
            ...
            # if args.dual_training:
            #     loss_q2d = outputs[1]["loss_q2d"]
            #     loss_d2q = outputs[1]["loss_d2q"]
            ...
            if args.gradient_accumulation_steps > 1:
                ...
                for k in loss_item_dict:
                    loss_item_dict[k] = loss_item_dict[k] / args.gradient_accumulation_steps
            ...
            tr_loss_dict = incremental_tr_loss(tr_loss_dict,loss_item_dict,total_loss=loss)
            ...
            #     if args.gradient_accumulation_steps > 1:
            #         loss_q2d = loss_q2d / args.gradient_accumulation_steps
            #         loss_d2q = loss_d2q / args.gradient_accumulation_steps
                # tr_loss += loss_q2d.item() # use the loss of primary task as main criteria
                # tr_loss_dual += loss_d2q.item()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    # if args.dual_training: 
                    #     # loss_scalar_dual = (tr_loss_dual - logging_loss_dual) / args.logging_steps
                    #     loss_scalar_total = (tr_loss_total - logging_loss_total) / args.logging_steps
                    #     logs["loss_dual"] = loss_scalar_dual
                    #     logs["loss_total"] = loss_scalar_total
                    #     logging_loss_dual = tr_loss_dual
                    #     logging_loss_total = tr_loss_total
...
def load_stuff(model_type, args):
    ...
    if model_type == "rdot_nll" and args.dual_training:
        configObj = MSMarcoConfig(name="rdot_nll",
                model=RobertaDot_NLL_LN,
                use_mean=False,
                dual_training_fn=args.dual_training,)
    else:
    ...
    # enable l2 normalization 
    if args.loss_objective_function == "simclr_cosine":
        ...
        logger.info(f"set model with normalized output, temperature {args.temperature}")
...
def get_arguments():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="primary task loss item weight",
    )
    parser.add_argument(
        "--dual_loss_weight",
        type=float,
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="The process number of pytorch dataloader",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="enable l2_normalization",
    )
    parser.add_argument(
        "--loss_objective_function",
        type=str,
        default="dot_product",
        choices=["dot_product","pairwise_hinge","simclr_cosine"],
        help="attention type",
    )
    parser.add_argument(
        "--temperature",
...
def main():
    ...
    if args.do_train:
        ...
        if args.dual_training:
            training_data_path= args.data_dir+"/quadruplets.train.small.filter_pos_qids.tsv"
        else:
            training_data_path= args.data_dir+"/triples.train.small.tsv"

        with open(training_data_path, encoding="utf-8" if args.dual_training else "utf-8-sig") as f:

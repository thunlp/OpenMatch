...
from data.msmarco_data import GetTrainingDataProcessingFn, GetTripletTrainingDataProcessingFn, \
    GetQuadrapuletTrainingDataProcessingFn, GetGroupedTrainingDataProcessingFn_polling,GetGroupedTrainingDataProcessingFn_origin
...
from model.models import MSMarcoConfigDict, ALL_MODELS, replace_model_with_spasre
...
def train(args, model, tokenizer, query_cache, passage_cache):
    ...
    is_hypersphere_training = (args.hyper_align_weight > 0 or args.hyper_unif_weight > 0)
    ...
    while global_step < args.max_steps:
        ...
        if step % args.gradient_accumulation_steps == 0 and global_step % args.logging_steps == 0:
            ...
            ann_no, ann_path, ndcg_json = get_latest_ann_data(args.ann_dir,is_grouped=(args.grouping_ann_data > 0))
            if ann_path is not None and ann_no != last_ann_no:
                ...
                logger.info("Total ann queries: %d", len(ann_training_data) if args.grouping_ann_data < 0 else len(ann_training_data)*args.grouping_ann_data)

                if args.grouping_ann_data > 0:
                    if args.polling_loaded_data_batch_from_group:
                        train_dataset = StreamingDataset(
                                ann_training_data, GetGroupedTrainingDataProcessingFn_polling(
                                    args, query_cache, passage_cache))
                    else:
                        train_dataset = StreamingDataset(
                                ann_training_data, GetGroupedTrainingDataProcessingFn_origin(
                                    args, query_cache, passage_cache))
                else:
                    if not args.dual_training:
                        if args.triplet:
                        ...
                        # return quadruplet
                        train_dataset = StreamingDataset(
                                ann_training_data, GetQuadrapuletTrainingDataProcessingFn(
                                    args, query_cache, passage_cache))
                ...
                if not args.single_warmup:
                    scheduler = get_linear_schedule_with_warmup(
                        optimizer,
                        num_warmup_steps=args.warmup_steps,
                        num_training_steps=len(ann_training_data) if args.grouping_ann_data < 0 else len(ann_training_data)*args.grouping_ann_data
                    )
                    ...
                    for key in ndcg_json:
                        if "marcodev" in key:
                            tb_writer.add_scalar(
                                key, ndcg_json[key], ann_checkpoint_no)
                                
                    if 'trec2019_ndcg' in ndcg_json:
                        tb_writer.add_scalar(
                            "trec2019_ndcg", ndcg_json['trec2019_ndcg'], ann_checkpoint_no)                        
        ...
        if args.grouping_ann_data <= 0:
            ...
            if args.dual_training:
                inputs["neg_query_ids"] = batch["q_neg"][0].to(args.device).long()
                inputs["attention_mask_neg_query"] = batch["q_neg"][1].to(args.device).long()
                inputs["prime_loss_weight"] = args.prime_loss_weight
                inputs["dual_loss_weight"] = args.dual_loss_weight      
...
def get_arguments():
    ...
    # -----------------  HyperSphere Property ------------------
    parser.add_argument(
        "--hyper_align_weight",
        type=float,
        default=0.0,
        help="hypershpere alignment property loss weight",
    )
    parser.add_argument(
        "--hyper_unif_weight",
        type=float,
        default=0.0,
        help="hypershpere uniformity property loss weight",
    )
    # ------------------- SimCLR parameters L2 normalization and Temperature------------------------
    parser.add_argument(
    ...
    parser.add_argument(
        "--polling_loaded_data_batch_from_group",
        default=False,
        action="store_true",
        help="if polling, return batches like [q1,q2,q3,q4]. otherwise remain the original way: [q1,q1,q1,q1], [q1,q1,q2,q2], [q2,q2,q2,q2]...",
    )    

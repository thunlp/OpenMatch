...
def preprocess(args):
    ...
    if os.path.isfile(out_passage_path):
        ...
        pid2offset,_ = load_mapping(args.out_data_dir, "pid2offset")
        # return
    else:
        out_line_count = 0
    ...
    write_qas_query(args, "nq-dev.csv", "dev-qas-query")
    write_qas_query(args, "trivia-dev.csv", "trivia-dev-qas-query")
...
# TODO: check
def GetQuadrapuletTrainingDataProcessingFn(args, query_cache, passage_cache, shuffle=True):
    ...
    def fn(line, i):
        ...
        # use one neg in DPR, so better to shuffle
        if shuffle:
            random.shuffle(neg_pids)
            random.shuffle(neg_qids)

        neg_d_data = GetProcessingFn(args, query=False)(passage_cache[neg_pids[0]], neg_pids[0])[0]
        neg_q_data = GetProcessingFn(args, query=True)(query_cache[neg_qids[0]],neg_qids[0])[0]
        yield (query_data[0], query_data[1], query_data[2], pos_data[0], pos_data[1], pos_data[2],
                neg_d_data[0], neg_d_data[1], neg_d_data[2], neg_q_data[0], neg_q_data[1], neg_q_data[2])

        # use all D- Q- in retrievavl for MSMARCO
        # for neg_pid,neg_qid in zip(neg_pids,neg_qids):
        #     neg_d_data = GetProcessingFn(args, query=False)(passage_cache[neg_pid], neg_pid)[0]
        #     neg_q_data = GetProcessingFn(args, query=True)(query_cache[neg_qid], neg_qid)[0]    
        #     yield (query_data[0], query_data[1], query_data[2], pos_data[0], pos_data[1], pos_data[2],
        #            neg_d_data[0], neg_d_data[1], neg_d_data[2], neg_q_data[0], neg_q_data[1], neg_q_data[2])

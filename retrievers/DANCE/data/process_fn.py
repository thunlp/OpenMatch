...
def quadruplet_process_fn(line, i, tokenizer, args):
    features = []
    cells = line.split("\t")
    if len(cells) == 4:
        # this is for training and validation
        # query, positive_passage, negative_passage, negative_query = line
        mask_padding_with_zero = True
        pad_token_segment_id = 0
        pad_on_left = False
        ...
        raise Exception(
            "Line doesn't have correct length: {0}. Expected 4.".format(str(len(cells))))
    return [features]
